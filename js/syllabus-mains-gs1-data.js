 ...
 * *** This version relies on the DOM and is for UI click events. ***
...
// --- *** NEW FUNCTION TO FIX THE COHERENCY BUG *** ---
/**
 * Recursively recalculates the status of parent nodes based on their children.
 * Operates directly on the in-memory syllabusData (post-order traversal).
 * This is used ONCE on page load to fix the initial state.
 * @param {object} node The syllabus node to process.
 */
function recalculateParentStatus(node) {
    if (!node) return null;

    // Check if it's a leaf node (micro-topic)
    if (!Array.isArray(node.children) || node.children.length === 0) {
        // Leaf nodes are the source of truth, return their existing status
        return node.status || STATUSES.NOT_STARTED;
    }

    // It's a parent node. Recurse on all children first.
    const childrenStatuses = node.children
        .map(child => recalculateParentStatus(child))
        .filter(status => status !== null); // Filter out any invalid child nodes

    if (childrenStatuses.length === 0) {
        // Parent with no valid children? Default to NOT_STARTED.
        node.status = STATUSES.NOT_STARTED;
        return node.status;
    }

    // Now, determine the parent's status from its children's statuses
    let newParentStatus;
    ...
    // Update the node's status in the data
    node.status = newParentStatus;

    // Return the newly calculated status
...
function updateTrackerDashboard() {
    ...
    // console.log("Calculating Mains GS3 Progress for:", mainsGS3Node?.children); // VerbVose log
...
function startOptionalSubjectListener(userId) {
    ...
    unsubscribeOptional = onSnapshot(profileDocRef, (docSnap) => {
        // Note: No need to check docSnap.exists() here, as the setDoc in the click handler
        // will create it, triggering this listener with the new data.
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    async function loadSyllabusData(userId) {
        ...
        try {
            ...
            // 6. *** *** FIX: Recalculate all parent statuses after merging *** ***
            console.log("Recalculating all parent statuses...");
            // This new function (defined above) traverses the *data* tree
            // and updates parent statuses in-memory.
    ...
    document.addEventListener('click', async function(e) {
        ...
        // Optional Subject Selection Button Click
            ...
            if (!firestoreModule?.doc || !firestoreModule?.setDoc) { showNotification("Database service unavailable.", true); return; } // *** FIX: Check for setDoc
            try { 
                // *** --------------------------------- ***
                // *** THIS IS THE FIX ***
                // *** --------------------------------- ***
                const { doc, setDoc } = firestoreModule; // Use setDoc
